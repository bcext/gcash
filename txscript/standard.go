...
// specified address.
func PayToAddrScript(addr cashutil.Address) ([]byte, error) {
...
func MultiSigScript(pubkeys []*cashutil.AddressPubKey, nrequired int) ([]byte, error) {
...
func ExtractPkScriptAddrs(pkScript []byte, chainParams *chaincfg.Params) (ScriptClass, []cashutil.Address, int, error) {
	...
	case PubKeyHashTy:
		...
		requiredSigs = 1
		addr, err := cashutil.NewAddressPubKeyHash(pops[2].data,
			chainParams)
		if err == nil {
	...
	case PubKeyTy:
		...
		requiredSigs = 1
		addr, err := cashutil.NewAddressPubKey(pops[0].data, chainParams)
		if err == nil {
			addrs = append(addrs, addr)
		}
	...
	case ScriptHashTy:
		...
		requiredSigs = 1
		addr, err := cashutil.NewAddressScriptHashFromHash(pops[1].data,
			chainParams)
		if err == nil {
			addrs = append(addrs, addr)
		}
	...
	case MultiSigTy:
		...
		addrs = make([]cashutil.Address, 0, numPubKeys)
		for i := 0; i < numPubKeys; i++ {
			addr, err := cashutil.NewAddressPubKey(pops[i+1].data,
				chainParams)
			if err == nil {
				addrs = append(addrs, addr)
			}
