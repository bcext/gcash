...
func mkGetKey(keys map[string]addressToKey) KeyDB {
	if keys == nil {
		return KeyClosure(func(addr cashutil.Address) (*btcec.PrivateKey,
			bool, error) {
	...
	return KeyClosure(func(addr cashutil.Address) (*btcec.PrivateKey,
		bool, error) {
		a2k, ok := keys[addr.EncodeAddress(true)]
		if !ok {
			return nil, false, errors.New("nope")
		}
...
func mkGetScript(scripts map[string][]byte) ScriptDB {
	if scripts == nil {
		return ScriptClosure(func(addr cashutil.Address) ([]byte, error) {
			return nil, errors.New("nope")
		})
	}
	return ScriptClosure(func(addr cashutil.Address) ([]byte, error) {
		script, ok := scripts[addr.EncodeAddress(true)]
		if !ok {
			return nil, errors.New("nope")
		}
		return script, nil
...
func signAndCheck(msg string, tx *wire.MsgTx, idx int, inputAmt int64, pkScript []byte,
	...
	sigScript, err := SignTxOutput(&chaincfg.TestNet3Params, tx, idx,
		pkScript, hashType, cashutil.Amount(inputAmt), kdb, sdb, nil)
	if err != nil {
...
func TestSignTxOutput(t *testing.T) {
	...
	for _, hashType := range hashTypes {
		for i := range tx.TxIn {
			...
			address, err := cashutil.NewAddressPubKeyHash(
				cashutil.Hash160(pk), &chaincfg.TestNet3Params)
			if err != nil {
			...
			if err := signAndCheck(msg, tx, i, inputAmounts[i], pkScript, hashType,
				mkGetKey(map[string]addressToKey{
					address.EncodeAddress(true): {key, false},
	...
	for _, hashType := range hashTypes {
		for i := range tx.TxIn {
			...
			sigScript, err := SignTxOutput(&chaincfg.TestNet3Params,
				tx, i, pkScript, hashType, cashutil.Amount(inputAmounts[i]),
				mkGetKey(map[string]addressToKey{
					address.EncodeAddress(true): {key, false},
				}), mkGetScript(nil), nil)
			if err != nil {
	...
	for _, hashType := range hashTypes {
		for i := range tx.TxIn {
			...
			if err := signAndCheck(msg, tx, i, inputAmounts[i],
				pkScript, hashType,
				mkGetKey(map[string]addressToKey{
					address.EncodeAddress(true): {key, true},
				}), mkGetScript(nil), nil); err != nil {
				t.Error(err)
				break
			}
	...
	for _, hashType := range hashTypes {
		for i := range tx.TxIn {
			...
			pk := (*btcec.PublicKey)(&key.PublicKey).
				SerializeUncompressed()
			address, err := cashutil.NewAddressPubKey(pk,
	...
	for _, hashType := range hashTypes {
		for i := range tx.TxIn {
			...
			scriptAddr, err := cashutil.NewAddressScriptHash(
			...
			if err := signAndCheck(msg, tx, i, inputAmounts[i],
				...
				mkGetKey(map[string]addressToKey{
					...
					scriptAddr.EncodeAddress(true): pkScript,
				}), nil); err != nil {
				t.Error(err)
				break
			}
	...
	for _, hashType := range hashTypes {
		for i := range tx.TxIn {
			...
			sigScript, err := SignTxOutput(&chaincfg.TestNet3Params,
				tx, i, scriptPkScript, hashType, cashutil.Amount(inputAmounts[i]),
				mkGetKey(map[string]addressToKey{
					address.EncodeAddress(true): {key, false},
				}), mkGetScript(map[string][]byte{
					scriptAddr.EncodeAddress(true): pkScript,
	...
	for _, hashType := range hashTypes {
		for i := range tx.TxIn {
			...
			pk1 := (*btcec.PublicKey)(&key1.PublicKey).
				SerializeCompressed()
			address1, err := cashutil.NewAddressPubKey(pk1,
				&chaincfg.TestNet3Params)
			if err != nil {
				t.Errorf("failed to make address for %s: %v",
					msg, err)
			...
			pk2 := (*btcec.PublicKey)(&key2.PublicKey).
				SerializeCompressed()
			address2, err := cashutil.NewAddressPubKey(pk2,
				&chaincfg.TestNet3Params)
			if err != nil {
			...
			pkScript, err := MultiSigScript(
				[]*cashutil.AddressPubKey{address1, address2},
				2)
			if err != nil {
				t.Errorf("failed to make pkscript "+
					"for %s: %v", msg, err)
			...
			if err := signAndCheck(msg, tx, i, inputAmounts[i],
				scriptPkScript, hashType,
				mkGetKey(map[string]addressToKey{
					address1.EncodeAddress(true): {key1, true},
					address2.EncodeAddress(true): {key2, true},
				}), mkGetScript(map[string][]byte{
					scriptAddr.EncodeAddress(true): pkScript,
				}), nil); err != nil {
				t.Error(err)
	...
	for i := range sigScriptTests {
		...
		for j := range tx.TxIn {
			...
			script, err = SignatureScript(tx, idx,
				...
				cashutil.Amount(sigScriptTests[i].inputs[j].txout.Value),
