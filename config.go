...
const (
	defaultConfigFilename        = "gcash.conf"
	defaultDataDirname           = "data"
	defaultLogLevel              = "info"
	defaultLogDirname            = "logs"
	defaultLogFilename           = "gcash.log"
	defaultMaxPeers              = 125
	...
	sampleConfigFilename         = "sample-gcash.conf"
	defaultTxIndex               = false
	defaultAddrIndex             = false
)

var (
	defaultHomeDir     = cashutil.AppDataDir("gcash", false)
...
// config defines the configuration options for gcash.
//
...
//
// The above results in gcash functioning properly without any config settings
...
func loadConfig() (*config, []string, error) {
	...
	if cfg.DisableRPC {
		cashLog.Infof("RPC service is disabled")
	}
	...
	if configFileError != nil {
		cashLog.Warnf("%v", configFileError)
	}

	return &cfg, remainingArgs, nil
}

// createDefaultConfig copies the file sample-gcash.conf to the given destination path,
...
// gcashDial connects to the address on the named network using the appropriate
// dial function depending on the address and configuration options.  For
// example, .onion addresses will be dialed using the onion specific proxy if
// one was specified, but will otherwise use the normal dial function (which
// could itself use a proxy or not).
func gcashDial(addr net.Addr) (net.Conn, error) {
...
// gcashLookup resolves the IP of the given host using the correct DNS lookup
...
func gcashLookup(host string) ([]net.IP, error) {
