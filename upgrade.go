...
// oldBtcdHomeDir returns the OS specific home directory gcash used prior to
...
func oldBtcdHomeDir() string {
	...
	if appData != "" {
		return filepath.Join(appData, "gcash")
	}

	// Fall back to standard HOME directory that works for most POSIX OSes.
	home := os.Getenv("HOME")
	if home != "" {
		return filepath.Join(home, ".gcash")
	}
...
// location prior to gcash version 0.2.0 and uses heuristics to ascertain the old
...
// upgradeDBPaths moves the databases from their locations prior to gcash
...
func upgradeDBPaths() error {
	...
	upgradeDBPathNet(filepath.Join(oldDbRoot, "gcash.db"), "mainnet")
	upgradeDBPathNet(filepath.Join(oldDbRoot, "gcash_testnet.db"), "testnet")
	upgradeDBPathNet(filepath.Join(oldDbRoot, "gcash_regtest.db"), "regtest")

	// Remove the old db directory.
	return os.RemoveAll(oldDbRoot)
}

// upgradeDataPaths moves the application data from its location prior to gcash
...
func upgradeDataPaths() error {
	...
	if fileExists(oldHomePath) && !fileExists(newHomePath) {
		// Create the new path.
		cashLog.Infof("Migrating application home path from '%s' to '%s'",
		...
		// Move old gcash.conf into new location if needed.
		...
		if ohpEmpty {
			...
			cashLog.Warnf("Not removing '%s' since it contains files "+
...
// doUpgrades performs upgrades to gcash as new versions require it.
