...
func (eft *estimateFeeTester) testTx(fee cashutil.Amount) *TxDesc {
	eft.version++
	return &TxDesc{
		TxDesc: mining.TxDesc{
			Tx: cashutil.NewTx(&wire.MsgTx{
				Version: eft.version,
			}),
			Height: eft.height,
			Fee:    int64(fee),
...
func (eft *estimateFeeTester) newBlock(txs []*wire.MsgTx) {
	...
	block := cashutil.NewBlock(&wire.MsgBlock{
		Transactions: txs,
	})
...
func (eft *estimateFeeTester) round(txHistory [][]*TxDesc,
	...
	for i := uint32(0); i < txPerRound; i++ {
		newTx := eft.testTx(cashutil.Amount(rand.Intn(1000000)))
