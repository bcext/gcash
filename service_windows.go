...
const (
	// svcName is the name of gcash service.
	svcName = "gcashsvc"
...
// logServiceStartOfDay logs information about gcash when the main server has
...
// gcashService houses the main service handler which handles all service
// updates and launching gcashMain.
type gcashService struct{}
...
// long-running gcashMain (which is the real meat of gcash), handles service
// change requests, and notifies the service control manager of changes.
func (s *gcashService) Execute(args []string, r <-chan svc.ChangeRequest, changes chan<- svc.Status) (bool, uint32) {
	// Service start is pending.
	const cmdsAccepted = svc.AcceptStop | svc.AcceptShutdown
	changes <- svc.Status{State: svc.StartPending}

	// Start gcashMain in a separate goroutine so the service can start
...
// installService attempts to install the gcash service.  Typically this should
...
// removeService attempts to uninstall the gcash service.  Typically this should
...
// startService attempts to start the gcash service.
...
func serviceMain() (bool, error) {
	...
	err = svc.Run(svcName, &gcashService{})
	if err != nil {
