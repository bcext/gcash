...
const (
	...
	baseSubsidy = 50 * cashutil.SatoshiPerBitcoin
)
...
func IsCoinBase(tx *cashutil.Tx) bool {
...
func IsFinalizedTransaction(tx *cashutil.Tx, blockHeight int32, blockTime time.Time) bool {
	msgTx := tx.MsgTx()
...
// difficulty is in min/max range and that the block hash is less than the
// target difficulty as claimed.
func CheckProofOfWork(block *cashutil.Block, powLimit *big.Int) error {
...
// txscript.
func GetSigOpCost(tx *cashutil.Tx) int {
	msgTx := tx.MsgTx()
...
func ExtractCoinbaseHeight(coinbaseTx *cashutil.Tx) (int32, error) {
...
func checkSerializedHeight(coinbaseTx *cashutil.Tx, wantHeight int32) error {
...
func CheckTransactionInputs(tx *cashutil.Tx, txHeight int32, utxoView *UtxoViewpoint, chainParams *chaincfg.Params) (int64, error) {
	...
	for txInIndex, txIn := range tx.MsgTx().TxIn {
		...
		if originTxSatoshi < 0 {
			str := fmt.Sprintf("transaction output has negative "+
				"value of %v", cashutil.Amount(originTxSatoshi))
			return 0, ruleError(ErrBadTxOutValue, str)
		}
		if originTxSatoshi > cashutil.MaxSatoshi {
			str := fmt.Sprintf("transaction output value of %v is "+
				"higher than max allowed value of %v",
				cashutil.Amount(originTxSatoshi),
				cashutil.MaxSatoshi)
			return 0, ruleError(ErrBadTxOutValue, str)
		}
		...
		if totalSatoshiIn < lastSatoshiIn ||
			totalSatoshiIn > cashutil.MaxSatoshi {
			str := fmt.Sprintf("total value of all transaction "+
...
func (b *BlockChain) CheckConnectBlockTemplate(block *cashutil.Block) error {
	b.chainLock.Lock()
