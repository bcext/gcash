...
// exceeded.
func (b *BlockChain) addOrphanBlock(block *cashutil.Block) {
...
//
// This function MUST be called with the chain state lock held (for writes).
func (b *BlockChain) connectBlock(node *blockNode, block *cashutil.Block,
...
//
// This function MUST be called with the chain state lock held (for writes).
func (b *BlockChain) disconnectBlock(node *blockNode, block *cashutil.Block, view *UtxoViewpoint) error {
...
func countSpentOutputs(block *cashutil.Block) int {
...
func (b *BlockChain) reorganizeChain(detachNodes, attachNodes *list.List) error {
	...
	detachBlocks := make([]*cashutil.Block, 0, detachNodes.Len())
	detachSpentTxOuts := make([][]SpentTxOut, 0, detachNodes.Len())
	attachBlocks := make([]*cashutil.Block, 0, attachNodes.Len())
...
type IndexManager interface {
	...
	// required.
	ConnectBlock(database.Tx, *cashutil.Block, []SpentTxOut) error

	// DisconnectBlock is invoked when a block has been disconnected from
	// the main chain. The set of outputs scripts that were spent within
	// this block is also returned so indexers can clean up the prior index
	// state for this block.
	DisconnectBlock(database.Tx, *cashutil.Block, []SpentTxOut) error
}
