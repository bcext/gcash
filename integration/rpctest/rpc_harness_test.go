...
func testSendOutputs(r *Harness, t *testing.T) {
	genSpend := func(amt cashutil.Amount) *chainhash.Hash {
	...
	// input.
	txid := genSpend(cashutil.Amount(5 * cashutil.SatoshiPerBitcoin))
	...
	txid = genSpend(cashutil.Amount(500 * cashutil.SatoshiPerBitcoin))
	blockHashes, err = r.Node.Generate(1)
	if err != nil {
		t.Fatalf("unable to generate single block: %v", err)
	}
...
func testGenerateAndSubmitBlock(r *Harness, t *testing.T) {
	...
	output := wire.NewTxOut(cashutil.SatoshiPerBitcoin, pkScript)

	const numTxns = 5
	txns := make([]*cashutil.Tx, 0, numTxns)
	for i := 0; i < numTxns; i++ {
		...
		txns = append(txns, cashutil.NewTx(tx))
	}
...
func testMemWalletReorg(r *Harness, t *testing.T) {
	...
	expectedBalance := cashutil.Amount(250 * cashutil.SatoshiPerBitcoin)
	...
	// chain.
	expectedBalance = cashutil.Amount(0)
	walletBalance = harness.ConfirmedBalance()
	if expectedBalance != walletBalance {
		t.Fatalf("wallet balance incorrect: expected %v, got %v",
			expectedBalance, walletBalance)
...
func testMemWalletLockedOutputs(r *Harness, t *testing.T) {
	...
	outputAmt := cashutil.Amount(50 * cashutil.SatoshiPerBitcoin)
...
func TestHarness(t *testing.T) {
	...
	// outputs.
	expectedBalance := cashutil.Amount(numMatureOutputs * 50 * cashutil.SatoshiPerBitcoin)
