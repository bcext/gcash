...
// Harness fully encapsulates an active gcash process to provide a unified
// platform for creating rpc driven integration tests involving gcash. The
// active gcash node will typically be run in simnet mode in order to allow for
// easy generation of test blockchains.  The active gcash process is fully
...
func New(activeNet *chaincfg.Params, handlers *rpcclient.NotificationHandlers,
	...
	if handlers.OnFilteredBlockConnected != nil {
		...
		handlers.OnFilteredBlockConnected = func(height int32, header *wire.BlockHeader, filteredTxns []*cashutil.Tx) {
...
func (h *Harness) SetUp(createTestChain bool, numMatureOutputs uint32) error {
	// Start the gcash node itself. This spawns a new process which will be
	// managed
	...
	// wallet.
	filterAddrs := []cashutil.Address{h.wallet.coinbaseAddr}
	if err := h.Node.LoadTxFilter(true, filterAddrs, nil); err != nil {
		return err
	}

	// Ensure gcash properly dispatches our registered call-back for each new
...
// connectRPCClient attempts to establish an RPC connection to the created gcash
...
func (h *Harness) NewAddress() (cashutil.Address, error) {
...
//
// This function is safe for concurrent access.
func (h *Harness) ConfirmedBalance() cashutil.Amount {
...
func (h *Harness) SendOutputs(targetOutputs []*wire.TxOut,
	feeRate cashutil.Amount) (*chainhash.Hash, error) {
...
func (h *Harness) CreateTransaction(targetOutputs []*wire.TxOut,
	feeRate cashutil.Amount, change bool) (*wire.MsgTx, error) {
...
//
// This function is safe for concurrent access.
func (h *Harness) GenerateAndSubmitBlock(txns []*cashutil.Tx, blockVersion int32,
	blockTime time.Time) (*cashutil.Block, error) {
...
func (h *Harness) GenerateAndSubmitBlockWithCustomCoinbaseOutputs(
	txns []*cashutil.Tx, blockVersion int32, blockTime time.Time,
	mineTo []wire.TxOut) (*cashutil.Block, error) {
	...
	prevBlock := cashutil.NewBlock(mBlock)
...
func baseDir() (string, error) {
	dirPath := filepath.Join(os.TempDir(), "gcash", "rpctest")
