...
5. [How do I use bootstrap.dat with gcash?](#Importing)
...
### 5. How do I use bootstrap.dat with gcash?

gcash comes with a separate utility named `addblock` which can be used to import
...
1. Stop gcash if it is already running.  This is required since addblock needs to
   access the database used by gcash and it will be locked if gcash is using it.
