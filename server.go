...
import (
	...
	"github.com/bcext/gcash/mining/cpuminer"
	"github.com/bcext/gcash/netsync"
...
func (sp *serverPeer) OnTx(_ *peer.Peer, msg *wire.MsgTx) {
	...
	// Convert the raw MsgTx to a cashutil.Tx which provides some convenience
	// methods and things such as hash caching.
	tx := cashutil.NewTx(msg)
...
func (sp *serverPeer) OnBlock(_ *peer.Peer, msg *wire.MsgBlock, buf []byte) {
	// Convert the raw MsgBlock to a cashutil.Block which provides some
	// convenience methods and things such as hash caching.
	block := cashutil.NewBlockFromBlockAndBytes(msg, buf)
...
func (s *server) TransactionConfirmed(tx *cashutil.Tx) {
...
func (s *server) peerHandler() {
	...
	if !cfg.DisableDNSSeed {
		...
		connmgr.SeedFromDNS(activeNetParams.Params, defaultRequiredServices,
			gcashLookup, func(addrs []*wire.NetAddress) {
...
// newServer returns a new gcash server configured to listen on addr for the
...
func newServer(listenAddrs []string, db database.DB, chainParams *chaincfg.Params, interrupt <-chan struct{}) (*server, error) {
	...
	amgr := addrmgr.New(cfg.DataDir, gcashLookup)
	...
	txC := mempool.Config{
		...
		CalcSequenceLock: func(tx *cashutil.Tx, view *blockchain.UtxoViewpoint) (*blockchain.SequenceLock, error) {
