...
Bitcoin RPC server that uses a gcash/bitcoin-abc compatible Bitcoin JSON-RPC
API.  This client has been tested with gcash (https://github.com/bcext/gcash),
cashwallet (https://github.com/bcext/cashwallet), and
bitcoin-abc (https://github.com/bitcoin-abc).

In addition to the compatible standard HTTP POST JSON-RPC API, gcash and
cashwallet provide a websocket interface that is more efficient than the standard
...
gcash or cashwallet by default.  However, configuration options are provided to
fall back to HTTP POST and disable TLS to support talking with inferior bitcoin-abc
style RPC servers.
...
In contrast, the websocket-based JSON-RPC interface provided by gcash and
cashwallet only uses a single connection that remains open and allows
...
All notifications provided by gcash require registration to opt-in.  For example,
...
example, the DebugLevel call is an extension only provided by gcash (and
cashwallet pass through).  Therefore if you call one of these commands against
...
Also, it is important to realize that gcash intentionally separates the wallet
functionality into a separate process named cashwallet.  This means if you are
connected to the gcash RPC server directly, only the RPCs which are related to
chain services will be available.  Depending on your application, you might only
need chain-related RPCs.  In contrast, cashwallet provides pass through treatment
...
The following full-blown client examples are in the examples directory:
 ...
 - bitcoincorehttp
   Connects to a bitcoin-abc RPC server using HTTP POST mode with TLS disabled
   and gets the current block count
 - gcashwebsockets
   Connects to a gcash RPC server using TLS-secured websockets, registers for
   block connected and block disconnected notifications, and gets the current
   block count
 - cashwalletwebsockets
   Connects to a cashwallet RPC server using TLS-secured websockets, registers
